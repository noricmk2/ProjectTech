...
namespace UnityEditor.TreeViewExamples
{
	...
	public class MyTreeAssetEditor : Editor
	{
		...
		void ToolBar ()
		{
			using (new EditorGUILayout.HorizontalScope ())
			{
				...
				if (GUILayout.Button ("Add Item", style))
				{
					...
					var element = new BehaviorTreeElement (BehaviorNodeType.SelectorNode, String.Empty, false, String.Empty, 0, 0, String.Empty, id);
		...
		class MyTreeView : TreeViewWithTreeModel<BehaviorTreeElement>
		{
			public MyTreeView(TreeViewState state, TreeModel<BehaviorTreeElement> model)
				: base(state, model)
			{
				showBorder = true;
