...
public class RecycleScroll : ScrollRect
{
    ...
    private bool m_IsReverse = true;
    private bool m_IsFirstUpdate;
    private InstantiateItemDelegate m_InstantDelegate;

    public LinkedList<RecycleSlotBase> SlotList => m_SlotList;
    ...
    public void Init(List<IRecycleSlotData> dataList, InstantiateItemDelegate instantDelegate, bool startFirst = true)
    {
        if (startFirst)
            m_StartIdx = 0;
        ...
        m_ViewportSize = GetSizeByCorner(corners);
        ...
        else
        {
            ...
            if (vertical)
            {
                var wCount = Mathf.CeilToInt((m_ViewportSize.x - (gridGroup.padding.left + gridGroup.padding.right)) / (m_ItemSize.x + gridGroup.spacing.x));
                var hCount = Mathf.CeilToInt((m_ViewportSize.y - (gridGroup.padding.bottom + gridGroup.padding.top)) / (m_ItemSize.y + gridGroup.spacing.y)) + 1;
            ...
            {
                var wCount = Mathf.CeilToInt((m_ViewportSize.x - (gridGroup.padding.left + gridGroup.padding.right)) / (m_ItemSize.x + gridGroup.spacing.x)) + 1;
                var hCount = Mathf.CeilToInt((m_ViewportSize.y - (gridGroup.padding.bottom + gridGroup.padding.top)) / (m_ItemSize.y + gridGroup.spacing.y));
                m_MaxVisibleCount = wCount * hCount;
        ...
        if (count > 0)
        {
            ...
            m_LastIdx = (count - 1) + m_StartIdx;
        }
        else
        {
            ObjectFactory.Instance.DeactivePoolObject(firstSlot);
        }
    }
    ...
    public void RefreshScroll(List<IRecycleSlotData> dataList = null)
    {
        if (dataList != null)
        {
            m_DataList = dataList;
            m_TotalCount = m_DataList.Count;
            int visibleCount = Math.Min(m_TotalCount, m_MaxVisibleCount);

            if (visibleCount > m_SlotList.Count)
            {
                int createCount = visibleCount - m_SlotList.Count;
                var idx = m_StartIdx;
            ...
            else
            {
                ...
                while (node != null)
                {
                    ...
                    else
                    {
                        ...
                        ObjectFactory.Instance.DeactivePoolObject(slot);
                    }
                    node = next;
                }
            }
    ...
    public void OnValueChange(Vector2 position)
    {
    ...
    private void SetSlot(bool setLast)
    {
        if (setLast)
        {
            for (int i = 0; i < m_ConstraintCount; ++i)
            {
                ...
                ObjectFactory.Instance.DeactivePoolObject(removeSlot);
                ++m_StartIdx;
    ...
    public void Release()
    {
        ...
        if (m_SlotList != null && m_SlotList.Count > 0)
        {
            ...
            while (iter.MoveNext())
            {
                poolName = iter.Current.name;
                iter.Current.Release();
                ObjectFactory.Instance.DeactivePoolObject(iter.Current);
            }
            ObjectFactory.Instance.Release();

            m_SlotList.Clear();
        }
        ObjectFactory.Instance.ReleaseOnePool(poolName);
